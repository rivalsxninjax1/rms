...
# Optional: canonical host redirect in production
CANONICAL_HOST = os.getenv("CANONICAL_HOST", "").strip()
if CANONICAL_HOST and not DEBUG:
    # insert near top (after SecurityMiddleware)
...
SESSION_COOKIE_AGE = int(os.getenv("SESSION_COOKIE_AGE", str(60 * 60 * 24 * 14)))  # 14 days by default
SESSION_SAVE_EVERY_REQUEST = False  # No need to rotate every request
...
CSRF_COOKIE_DOMAIN = os.getenv("CSRF_COOKIE_DOMAIN", None) or None
...
# ---------------- CORS / CSRF ----------------
# Dev: allow all origins; Prod: default False (configure CORS_ALLOWED_ORIGINS if needed)
CORS_ALLOW_ALL_ORIGINS = True if DEBUG else False

# SITE_URL is used to build CSRF_TRUSTED_ORIGINS correctly with scheme
SITE_URL = os.getenv("SITE_URL", "http://localhost:8000").rstrip("/")
DOMAIN = os.getenv("DOMAIN", SITE_URL).rstrip("/")

# CSRF_TRUSTED_ORIGINS must include scheme in Django 4+
_csrf_origins = set()
for val in _split_list("DJANGO_CSRF_TRUSTED_ORIGINS", ""):
    _csrf_origins.add(val.rstrip("/"))
if SITE_URL:
    _csrf_origins.add(SITE_URL)
# Also add https version of DOMAIN if provided without scheme
parsed_dom = urlparse(DOMAIN if "://" in DOMAIN else f"http://{DOMAIN}")
if parsed_dom.netloc:
    _csrf_origins.add(f"http://{parsed_dom.netloc}")
    _csrf_origins.add(f"https://{parsed_dom.netloc}")
...
# ---------------- Aggregators (set in .env if you use them) ----------------
