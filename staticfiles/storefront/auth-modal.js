 ...
 * Card-style auth modal + clean UX.
 */
(function () {
  ...
  function openModal(which="choice") { modal.classList.remove("hidden"); show(which); }
  ...
  async function jwtLogin(username, password) {
    ...
    // Store tokens (fallback if your app.js doesn't expose auth.set)
    if (window.auth && typeof window.auth.set === "function") {
      window.auth.set(data.access, data.refresh);
    } else {
  ...
  loginForm?.addEventListener("submit", async (e) => {
    ...
    try {
      await jwtLogin(form.get("username"), form.get("password"));
      await createSessionAndMerge();
      loginStatus.textContent = "Success!";
      closeModal();
      await afterAuthContinueCheckout();
    } catch (err) {
  ...
  signupForm?.addEventListener("submit", async (e) => {
    ...
    try {
      const r = await fetch("/accounts/auth/register/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      await jwtLogin(payload.username, payload.password);
      await createSessionAndMerge();
      signupStatus.textContent = "Account created!";
      closeModal();
      await afterAuthContinueCheckout();
    } catch (err) {
      signupStatus.textContent = (err && err.message) || "Signup failed.";
